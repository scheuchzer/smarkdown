...
public class Strapdown2ScriptProvider {
	...
	public void onEvent(@Observes ScriptEvent event) {
		...
		ValueExpression expression = FacesContext.getCurrentInstance().getApplication().getExpressionFactory().createValueExpression(elContext, "${request.contextPath}", String.class);
		Object path = expression.getValue(elContext);
		String contextPath = String.format(src, path);
		event.registerEndOfBodyScriptscript(contextPath);
	}
}
